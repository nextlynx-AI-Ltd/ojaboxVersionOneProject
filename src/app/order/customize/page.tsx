...
import { ArrowLeft, Plus, Minus, Trash2, ShoppingBasket, PlusCircle, Info } from "lucide-react"
import Image from "next/image"
import Link from "next/link"
import { Button } from "@/components/ui/button"
import { packs, allItems, formatPrice, FoodItem, categoryLabels, ItemCategory } from "@/data/packs"
import { useOrder, CartItem } from "@/context/OrderContext"

const CATEGORY_ORDER: ItemCategory[] = ["staples", "quick-foods", "cooking-essentials", "extras"]
    ...
    <div className="min-h-screen flex flex-col pb-24 lg:pb-0">
      <div className="container py-8 md:py-12 flex-1">
        ...
        <div className="rounded-xl bg-primary/5 border border-primary/10 p-4 md:p-5 mb-8 flex items-start gap-3">
          ...
          <div>
            <p className="font-medium text-sm mb-1">Customize your {pack.name}</p>
            <p className="text-xs text-muted-foreground leading-relaxed">
              Adjust quantities within the recommended range for this pack.
              Your total updates automatically as you make changes.
            </p>
          </div>
        </div>
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground">
                              {ci.item.unit} · {formatPrice(ci.item.pricePerUnit)} each
                            </p>
                            {atMax && (
                              <p className="text-xs text-destructive/80 mt-0.5">Max {ci.max} for this pack</p>
                            )}
                          </div>
            ...
            {availableToAdd.length > 0 && (
              <div className="mt-2">
                {!showAdd ? (
                  ...
                  >
                    <PlusCircle className="h-4 w-4" /> Add more items to your box
                  </button>
                ) : (
                  <div className="border rounded-xl p-4 md:p-5 bg-card">
                    <div className="flex items-center justify-between mb-4">
                      <p className="font-medium text-sm">Available items</p>
                      <button onClick={() => setShowAdd(false)} className="text-xs text-muted-foreground hover:text-foreground">Close</button>
                    </div>
                    {CATEGORY_ORDER.map((cat) => {
                      ...
                      return (
                        <div key={cat} className="mb-4 last:mb-0">
                          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">{categoryLabels[cat]}</p>
                            ...
                            {catItems.map((item) => (
                              ...
                              >
                                ...
                                <div className="flex items-center gap-2">
                                  <span className="text-sm font-semibold">{formatPrice(item.pricePerUnit)}</span>
          ...
          <div className="lg:col-span-1 hidden lg:block">
            <div className="sticky top-24 rounded-xl border bg-card p-6">
              ...
              <div className="space-y-2 mb-4 text-sm max-h-64 overflow-y-auto">
                {items.map((ci) => (
                  <div key={ci.item.id} className="flex justify-between text-muted-foreground">
              ...
              <div className="border-t pt-4 mb-6">
                ...
                <p className="text-xs text-muted-foreground mt-1">Delivery fee added at checkout</p>
              </div>
              ...
              <div className="mt-4 pt-4 border-t">
                <div className="flex items-start gap-2 text-xs text-muted-foreground">
                  ...
                  <span>Secure checkout. Pay with card, bank transfer, or USSD via Paystack.</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-card border-t p-4 lg:hidden z-40">
        <div className="container flex items-center justify-between gap-4">
          ...
          <Button size="lg" onClick={handleCheckout} disabled={items.length === 0} className="px-8">
            Review Order
          </Button>
        </div>
      </div>
